...
def progress(done,total,started):
 elapsed=max(time.monotonic()-started,.001); speed=done/elapsed/(1024*1024); pct=(done/total*100) if total else 100
 width=28; filled=int(width*pct/100); bar="█"*filled+"░"*(width-filled); eta=((total-done)/(speed*1024*1024)) if speed>0 and total else 0
...
def send_file(host,port,file_path,password):
 ...
 size=file_path.stat().st_size; aes=AESGCM(derive_key(password,os.urandom(SALT_SIZE))); salt=None
 # Derive once from the same salt sent on the wire.
 salt=os.urandom(SALT_SIZE); aes=AESGCM(derive_key(password,salt)); digest=file_sha256(file_path)
